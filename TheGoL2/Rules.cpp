...
void Ruler::func_reader(Board* board) { //исполнение команд
    ...
    while (!flag) {
        ...
        if (comand == "help" || comand == "?") { //подсказки для ввода
            std::cout << '\n';
        ...
        else if (comand == "dump") { //сохранение Вселенной
            std::cout << "Введите имя входного файла" << '\n';
            std::string filename;
            std::getline(std::cin, filename);
            board->save_universe(filename, name, rules);
        }
        else if (comand.substr(0, 5) == "dump ") { //сохранение Вселенной по названию
            std::string filename = comand.substr(5);
            board->save_universe(filename, name, rules);
        }
        else if (comand == "exit") { //выход
            flag = true;
        }
        else if (comand == "t" || comand == "tick") { //результат через одну итерацию
            board->step();
            std::cout << name << '\n' << rules << '\n' << board->get_number_loops() << '\n';
            board->show_board();
        }
        else if (comand.substr(0, 2) == "t ") { //результат через н итерацию
